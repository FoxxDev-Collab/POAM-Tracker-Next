...
export default async function GroupDetailPage({ params }: { params: Promise<{ id: string; gid: string }> }) {
    ...
    return (
      <AppShell>
        <div className="max-w-4xl mx-auto space-y-4 p-6">
          ...
          <Link href={`/packages/${packageId}`}><Button variant="outline">Back to Package</Button></Link>
        </div>
      </AppShell>
    );
  }

  const systems = Systems.byGroup(gid);

  // Aggregated STIG stats across all systems in this group
  const totalRow = getDb().prepare(`
    SELECT COUNT(*) as c
    FROM stig_findings f
    JOIN systems s ON s.id = f.system_id
    WHERE s.group_id = ?
  `).get(gid) as { c: number } | undefined
  const totalFindings = totalRow?.c ?? 0

  const bySeverity = getDb().prepare(`
    SELECT COALESCE(severity, 'unknown') as k, COUNT(*) as c
    FROM stig_findings f
    JOIN systems s ON s.id = f.system_id
    WHERE s.group_id = ?
    GROUP BY k
  `).all(gid) as { k: string; c: number }[]
  ...
  const latestScan = getDb().prepare(`
    ...
    LIMIT 1
  `).get(gid) as { id: number; system_id: number; title: string | null; checklist_id: string | null; created_at: string } | undefined
  ...
  return (
    <AppShell>
      <div className="p-6 grid gap-6">
              ...
              {latestScan ? (
                <>
                  ...
                  <div className="flex justify-end">
                    <Button asChild variant="outline" size="sm">
                      <Link href={`/systems/${latestScan.system_id}/stig`}>
        ...
        <GroupSystemsManager gid={gid} initial={systems as GroupSystemItem[]} />
      </div>
    </AppShell>
  );
}
