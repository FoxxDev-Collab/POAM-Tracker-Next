...
export default function GroupSystemsManager({ gid, initial }: { gid: number; initial: SystemItem[] }) {
  ...
  const reload = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetch(`/api/groups/${gid}/systems`, { cache: "no-store" })
  ...
  const onCreate = async (values: FormOutput) => {
    const res = await fetch(`/api/groups/${gid}/systems`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(values) })
    if (!res.ok) {
      const j = await res.json().catch(() => ({}))
      alert(j.error ?? "Failed to create system in group")
      return
    }
  ...
  return (
    <div className="grid gap-6">
      <Card>
        <CardHeader>
          <CardTitle>Add System to this Group</CardTitle>
        </CardHeader>
        <CardContent>
          <Form {...form}>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex-1">
                    ...
                    <FormControl>
                      <Input placeholder="e.g., Web Node 01" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
      ...
      <Card>
        <CardHeader>
          <CardTitle>Systems in this Group</CardTitle>
        </CardHeader>
        <CardContent>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left border-b">
                    <th className="py-2 pr-3 w-[35%]">System</th>
                    <th className="py-2 pr-3 w-[100px]">Total</th>
                    <th className="py-2 pr-3 w-[100px]">High</th>
                    <th className="py-2 pr-3 w-[100px]">Medium</th>
                    <th className="py-2 pr-3 w-[100px]">Low</th>
                    <th className="py-2 pr-2 w-[220px]">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {items.map((s) => (
